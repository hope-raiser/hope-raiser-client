...
const Home = ({ query }) => {
		...
		for (let i = 0; i < campaigns.totalPages; i++) {
			pagination.push(
				<Text
					...
					bg={
						campaigns.currentPage === i + 1
							? "teal.500"
							: "gray.200"
					}
					color={
						campaigns.currentPage === i + 1 ? "white" : "gray.700"
					}
					key={i}
	...
	const prevPage = () => {
		const prev =
			campaigns.currentPage === 1 ? "" : campaigns.currentPage - 1;
	...
	const nextPage = () => {
		const next =
			campaigns.currentPage === campaigns.totalPages
				? campaigns.currentPage + 0
	...
	return (
		<>
			<Layout user={userData}>
				...
				<section
					className=" py-24 px-4 md:px-12  border-b border-slate-200"
					id="campaign"
				>
					<div className="container  mx-auto">
						<h1 className="text-xl lg:text-3xl 2xl:text-4xl font-semibold text-center mb-8">
							Explore Campaigns
						</h1>
				...
				<section className="py-24 px-8 md:px-28 ">
					<div className="container mx-auto  max-w-[1400px]">
						<div className="flex flex-wrap border-b-2 items-center border-slate-200 justify-around">
							<h1 className="text-xl lg:text-5xl max-w-sm font-bold pb-8">
								WHY HopeRaiser?
							</h1>
